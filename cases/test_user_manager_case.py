...
class TestUserManagerTest(unittest.TestCase):
    ...
    def test02_edit_user(self):
        ...
        #2、调用编辑接口
        result = self.user.edit_user(TestUserManagerTest.user_id,new_user,password)  #注意类变量user_id的写法，要通过类名调用
        #3、进行断言
        self.assertEqual(0,result.get('errno'))
    ...
    def test04_delete_user(self):
        ...
        # 2、调用删除接口
        result = self.user.delete_user(TestUserManagerTest.user_id, new_user, password)  # 注意类变量user_id的写法，要通过类名调用
        # 3、进行断言
        self.assertEqual(0, result.get('errno'))
